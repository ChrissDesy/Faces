...
public class FacePanel extends JPanel implements Runnable
{
  ...
  // return an image
  {
    ...
    try {
       image = ImageIO.read(getClass().getClassLoader().getResource("media/"+imFnm));
       //image = ImageIO.read( new File("src/media/"+imFnm) );   // read in as an image
  ...
  private void initDetector()
  {
   ...
   if (classifier.isNull()) {
      System.out.println("\nCould not load the classifier file: " + FACE_CASCADE_FNM+"\n\t Copying alternate file");
      //String filepath = extractHaar.extract(FACE_CASCADE_FNM);
      //copyFile();
      System.out.println("Tryin first alternative..");
      classifier = new CvHaarClassifierCascade(cvLoad(ClassLoader.getSystemResource("media/"+FACE_CASCADE_FNM).toString()));
      if(classifier.isNull()){classifier = new CvHaarClassifierCascade(cvLoad("C:/" + FACE_CASCADE_FNM));}
      if (classifier.isNull()) JOptionPane.showMessageDialog(null,"Face Detection File Not Found");
...
*/
  ...
  */
  ...
  private void saveClip(BufferedImage clipIm) throws IOException
     ...
     clips to an exact size, then saves in a standard location */
  {
    ...
    if(new File(FACE_DIR).exists() == false){
        Files.createDirectories(Paths.get(FACE_DIR));
    }
    //Files.createDirectory(Paths.get(FACE_DIR));
  ...
  // save image in fnm
  {
    ...
    private boolean copyFile(){
        ...
        try{
            File srcc = new File(getClass().getClassLoader().getResource("media/"+FACE_CASCADE_FNM).toURI().toURL().toString());
            System.out.println("found File\n"+srcc);
            File targett = new File(FACE_CASCADE_FNM);
            System.out.println("found File\n"+targett);
            Files.copy(srcc.toPath(), targett.toPath(), StandardCopyOption.REPLACE_EXISTING);
