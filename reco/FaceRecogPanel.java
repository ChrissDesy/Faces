...
public class FaceRecogPanel extends JPanel implements Runnable {
    ...
    private BufferedImage loadImage(String imFnm) // return an image
    {
        ...
        try {
            image = ImageIO.read(getClass().getClassLoader().getResource("media/" + imFnm));   // read in as an image
    ...
    private void initDetector() {
        ...
        classifier = new CvHaarClassifierCascade(cvLoad("src/media/" + FACE_CASCADE_FNM));
        if (classifier.isNull()) {
            System.out.println("\nCould not load the classifier file: " + FACE_CASCADE_FNM + "\n\t Using alternate file");
            classifier = new CvHaarClassifierCascade(cvLoad("C:/" + FACE_CASCADE_FNM));
            //System.exit(1);
        }
    ...
    private void writeName(Graphics2D g2) /* write the currently recognized face name */ {
        g2.setColor(Color.YELLOW);
        g2.setFont(msgFont);
        if (faceName != null)     // draw at bottom middle
          g2.drawString("FACE RECOGNIZED.. please wait....", WIDTH/2, HEIGHT-10);  
    }  // end of writeName()

    private void writeStats(Graphics2D g2) /* write statistics in bottom-left corner, or
    ...
    // ------------------------- face tracking ----------------------------
    private void trackFace(final IplImage img) /* Create a separate thread for the time-consuming detection and recognition tasks:
    ...
    private CvRect findFace(IplImage grayIm) /* The Haar detector is a JavaCV function, so requires an IplImage object.
     ...
     */ {
	 ...
	 debugCanvas.waitKey(0);
        ...
        {
            JOptionPane.showMessageDialog(null, "Multiple faces detected (" + total + "); using the first");
        } else {
    ...
    private void recogFace(IplImage img) /* clip the image using the current face rectangle, then try to recognize it.
    ...
    private void matchClip(BufferedImage clipIm) // resize, convert to grayscale, clip to FACE_WIDTH*FACE_HEIGHT, recognize
    {
        long startTime = System.currentTimeMillis();
        ...
        if (result == null) {
            ...
            float cd = Float.parseFloat(distStr);
            if (cd <= 0.8) {
                controller.setemid(faceName);
                new employee().show();
                top.vhara(); 
                System.out.println("  Matches " + result.getMatchFileName()
                        + "; distance = " + distStr);
                System.out.println("  Matched name: " + result.getName());
                top.setRecogName(result.getName(), distStr);
            } else {
                System.out.println("No Match Found For This Face");
                //JOptionPane.showMessageDialog(null, "No Match Found For This Face\nTRY AGAIN");
            }
        }
        System.out.println("Match time: " + (System.currentTimeMillis() - startTime) + " ms");
    }  // end of matchClip()

    private BufferedImage resizeImage(BufferedImage im) /* resize so *at least* FACE_WIDTH*FACE_HEIGHT size
